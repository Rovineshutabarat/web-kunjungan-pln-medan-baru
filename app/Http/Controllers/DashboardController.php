...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $totalVisitsToday = Visit::whereDate('visit_date', Carbon::today())->count();
        $totalVisitsMonth = Visit::whereBetween('visit_date', [$currentMonthStart, $currentMonthEnd])->count();
        $totalVisitsPreviousMonth = Visit::whereBetween('visit_date', [$previousMonthStart, $previousMonthEnd])->count();

        $growthPercentage = 0;
        if ($totalVisitsPreviousMonth > 0) {
            $growthPercentage = (($totalVisitsMonth - $totalVisitsPreviousMonth) / $totalVisitsPreviousMonth) * 100;
        }
        ...
        $recentVisits = Visit::with('visitor')
            ...
            ->map(function ($visit) {
                return [
                    ...
                    'check_in' => $visit->check_in ? Carbon::parse($visit->check_in)->format('H:i') : '-',
                    'check_out' => $visit->check_out ? Carbon::parse($visit->check_out)->format('H:i') : null,
                    'status' => $visit->status,
        ...
        $topVisitorsData = Visit::select('visitor_id', DB::raw('count(*) as visit_count'))
            ->with('visitor')
        ...
        $topVisitors = $topVisitorsData->map(function ($item) use ($totalVisitorCount) {
            $percentage = $totalVisitorCount > 0 ? round(($item->visit_count / $totalVisitorCount) * 100) : 0;
            return [
