...
@section('admin-content')
    <div>
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Registrasi Kunjungan</h1>
            <p class="text-gray-600 mt-2">Lengkapi formulir di bawah untuk mendaftarkan kunjungan Anda</p>
        </div>

        <form action="{{ route('adminpage.visit.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="m-6">
                <div class="flex flex-col sm:flex-row gap-4">
                    <label
                        class="flex-1 cursor-pointer border rounded-xl p-4 flex items-center transition-shadow hover:shadow-lg bg-white shadow-sm">
                        <input type="radio" name="visitor_type" value="existing" class="form-radio h-5 w-5 text-blue-600"
                            checked onclick="toggleVisitor(true)">
                        <span class="ml-3 text-gray-800 font-medium">Gunakan Pengunjung Lama</span>
                    </label>
                    <label
                        class="flex-1 cursor-pointer border rounded-xl p-4 flex items-center transition-shadow hover:shadow-lg bg-white shadow-sm">
                        <input type="radio" name="visitor_type" value="new" class="form-radio h-5 w-5 text-blue-600"
                            onclick="toggleVisitor(false)">
                        <span class="ml-3 text-gray-800 font-medium">Buat Pengunjung Baru</span>
                    </label>
                </div>
            </div>

            <div id="existing-visitor" class="m-6 mb-0">
                <label class="block text-sm font-medium text-gray-700 mb-2">Pilih Pengunjung</label>
                <select name="existing_visitor_id"
                    class="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-200 bg-gray-50 hover:bg-white border-gray-200 focus:border-blue-500 transition-all">
                    <option value="">-- Pilih Pengunjung --</option>
                    @foreach ($visitors as $v)
                        <option value="{{ $v->id }}">{{ $v->full_name }} | {{ $v->identity_number }}</option>
                    @endforeach
                </select>
            </div>


            <div id="new-visitor-fields"
                class="p-8 pt-0 border-b border-gray-100 bg-gradient-to-br from-gray-50/50 to-white">
            ...
            <div class="p-8 border-b border-gray-100 bg-gradient-to-br from-gray-50/50 to-white">
                ...
                <div id="documentContainer" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-camera text-gray-500 mr-2"></i>Foto Selfie <span
                                class="text-red-500">*</span>
                        </label>
                        <input type="file" id="selfie" name="selfie_image" accept="image/*" class="hidden"
                            onchange="previewImage(event, 'selfie-preview')">
...
@push('scripts')
    <script>
        ...
        function toggleVisitor(isExisting) {
            document.getElementById('existing-visitor').style.display = isExisting ? 'block' : 'none';
            document.getElementById('new-visitor-fields').style.display = isExisting ? 'none' : 'block';
            document.getElementById('idCardContainer').style.display = isExisting ? 'none' : 'block';

            const documentContainer = document.getElementById('documentContainer');
            if (isExisting) {
                documentContainer.classList.remove('md:grid-cols-2');
                documentContainer.classList.add('grid-cols-1');
            } else {
                documentContainer.classList.remove('grid-cols-1');
                documentContainer.classList.add('md:grid-cols-2');
            }
        }
